...
//-------------Global Variables--------------
int i,j,n,k;
const int imax=15;//knapsacks in a set
const int jmax=110;//items in a set
const int nmax=1;
...
for (i=0;i<imax;i++){
	...
	for (j=0;j<jmax;j++){	
        ...
        IloNumVar X(env,0,1,ILOINT,ItemChoice); 
        Xj.add(X);
	}
	Xij.add(Xj);
}
...
for (i=0;i<imax;i++){
	...
	double LBKnapCapi=-IloInfinity,UBKnapCapi=0;
	IloRange KnapCap(env,LBKnapCapi,expr,UBKnapCapi,KnapsackCapacity);
	modelMaster_ptr.add(KnapCap);
...
for (j=0;j<jmax;j++){
	...
	modelMaster_ptr.add(ItemOneKnapsack);
...
while ((MasterObj-LowerBoundGlobal)/MasterObj>gap && local_loop<10000){	
	...
	if(local_loop>1 || NodeNumber==0){//if we are not in the first iteration or we are not in the first iteration of the first(ROOT) node(ParentBound=MasterObj=0)
		...
		MasterObj=parent_node->nodeUbound;
		for (i=0;i<imax;i++){
			YiValue[i]=parent_node->YiValueNode[i];
		}
		
		ThetaValue=parent_node->ThetaValueNode;
		OptimalThetaValue=ThetaValue;
...
int PrintOptimalSolution(){
	std::ostringstream os;
	os << "C:\\Code_Results\\ExtraConstraintRandom\\CPLEX\\CPLEX - OptimalSolution(i" << imaxstring << ",j"<<jmaxstring<<").txt";
	std::string FileName = os.str();
	...
	*/
	fsOptimalSolution<<"OptimalObjFunction="<<MasterObj<< std::endl;
	...
	*/
	...
	for (i=0;i<imax;i++){
		if(YiValue[i]>0.01){
			fsOptimalSolution<<"OptimalYiValue"<<"["<<i<<"]="<<YiValue[i]<<std::endl;
		}	
	}
	fsOptimalSolution<<"----------------------------------"<< std::endl;
	for (i=0;i<imax;i++){
		for (j=0;j<jmax;j++){
			if(XijValue[i][j]>0.01){
				fsOptimalSolution<<"OptimalXijValue"<<"["<<i<<"]"<<"["<<j<<"]="<<XijValue[i][j]<<std::endl;
			}
		}
	}
...
/*
	...
	Iterations << "C:\\Code_Results\\ExtraConstraintRandom\\KnapsackResults(i" << imaxstring << ",j"<<jmaxstring<<")\\With LOCAL CUTS - IterationsNumber(i" << imaxstring << ",j"<<jmaxstring<<").txt";
	...
	FeasibilityCuts << "C:\\Code_Results\\ExtraConstraintRandom\\KnapsackResults(i" << imaxstring << ",j"<<jmaxstring<<")\\With LOCAL CUTS - BendersFEASCutsNumber(i" << imaxstring << ",j"<<jmaxstring<<").txt";
	...
	OptimalityCuts << "C:\\Code_Results\\ExtraConstraintRandom\\KnapsackResults(i" << imaxstring << ",j"<<jmaxstring<<")\\With LOCAL CUTS - BendersOPTCutsNumber(i" << imaxstring << ",j"<<jmaxstring<<").txt";
	...
	LowerBound << "C:\\Code_Results\\ExtraConstraintRandom\\KnapsackResults(i" << imaxstring << ",j"<<jmaxstring<<")\\With LOCAL CUTS - BBLowerBound(i" << imaxstring << ",j"<<jmaxstring<<").txt";
	...
	UpperBound << "C:\\Code_Results\\ExtraConstraintRandom\\KnapsackResults(i" << imaxstring << ",j"<<jmaxstring<<")\\With LOCAL CUTS - BBUpperBound(i" << imaxstring << ",j"<<jmaxstring<<").txt";
	...
	TimePath << "C:\\Code_Results\\ExtraConstraintRandom\\KnapsackResults(i" << imaxstring << ",j"<<jmaxstring<<")\\With LOCAL CUTS - Time(i" << imaxstring << ",j"<<jmaxstring<<").txt";
...
*/
	...
	for (i=0;i<imax;i++){
		YiValue[i]=cplexMaster_ptr.getValue(Yi[i]);
		for (j=0;j<jmax;j++){
			XijValue[i][j]=cplexMaster_ptr.getValue(Xij[i][j]);
